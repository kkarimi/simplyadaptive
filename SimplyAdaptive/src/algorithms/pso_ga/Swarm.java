...
public class Swarm {
	...
	PSOFitnessFunction fitnessFunction;
	 ...
	 */
	public Swarm(int numberOfParticles, Particle sampleParticle, PSOFitnessFunction fitnessFunction) {
	...
	public PSOFitnessFunction getFitnessFunction() {
	...
	public void setFitnessFunction(PSOFitnessFunction fitnessFunction) {
		this.fitnessFunction = fitnessFunction;
	}
	...
	public void seedPosition( double[] values) {
		for(int  i=0;i<this.particles.length/2; i++){
		//for(Particle p:this.particles){
			this.particles[i].position=values.clone();
		}
	}
	...
	public void show(Graphics graphics, Color foreground, int width, int height, int dim0, int dim1, boolean showVelocity) {
		...
		if( particles != null ) {
			...
			for( int i = 0; i < particles.length; i++ ) {
				...
				if( showVelocity ) { //show trembling
					graphics.setColor(Color.PINK);
					graphics.drawLine(x, y, x, y-t);
				}
			}
			//draw best position
			{
				...
				int vx, vy, x, y;
				double pos[] = this.getBestPosition();
				x = (int) (scalePosW * (pos[dim0] - minPosW));
				y = height - (int) (scalePosH * (pos[dim1] - minPosH));
				graphics.drawRect(x - 1, y - 1, 3, 3);
