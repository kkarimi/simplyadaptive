...
// the dataset was created based on an utility model { 0.6, 0.1, 0.3 }, with trembling hand = 0.1
	...
	double[][][] variables={
			{{0.55,0.98,0.13},{0.60,0.88,0.15},{0.33,0.94,0.02},{0.39,0.27,0.55}}
...
//SETUP SWARM	
		...
		swarm.setParticleUpdate(new ParticleUpdateMixed(new Particle(variableCount+1), 0.0,0.01,0.0));
...
/*				swarm.setMaxPosition(max);
		...
		//debug - verify final solution using Monte Carlo simulation
		{
			double maxF=0;
			for(int i=0; i<1000;i++){  
				//System.out.println(fitnessFunction.evaluate(swarm.getBestPosition()));
				double f=fitnessFunction.evaluate(swarm.getBestPosition());
				if(f > maxF) maxF=f;
			} //trembling hand ... oh well
	...
	public static void main(String[] args) {
		...
		Example_MaxUtil example=new Example_MaxUtil();	
		//train model
